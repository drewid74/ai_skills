...
Use this when: design an agent harness, what goes in AGENTS.md, my agent keeps doing the wrong things, set up workspace isolation for agents, agent ignores my rules, how to structure agent instructions, prevent agent drift, agent keeps breaking things, set up CI for agent PRs, my agent breaks production, multi-agent coordination, how to write AGENTS.md, agents.md structure, agent hooks, PreToolUse hook, PostToolUse hook, agent entropy, garbage collection for agent code, golden principles for agents, blast radius control, agent loop termination, agent state persistence, checkpoint resume, generator evaluator pattern, eval harness CI gate, authority tier model, auto_safe proposed blocked, Symphony harness, codex harness, WORKFLOW.md design, harness vs agent loop, minimum viable harness, production harness, agent guardrails, agent permission model, tool registry, execution context, harness engineering checklist, context window engineering, progressive disclosure AGENTS.md, knowledge base docs structure, mechanical enforcement linters hooks, agent observability tracing, cost controls circuit breaker, human in the loop escalation.
